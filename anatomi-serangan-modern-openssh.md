...
------------------------------------------------------------------------
  ...
  Serangan modern bergerak ke tempat yang lebih sulit dilihat: signal handler,
  dependency chain, build artifact, distro-specific patch, client-side expansion,
  forwarding path, dan transisi kriptografi baru. Fokus serangan bergeser dari
  file sshd itu sendiri ke komponen yang berada di sekitar sshd.
...
------------------------------------------------------------------------
  ...
  Scope artikel ini:
    ...
    1. Membahas OpenSSH sebagai ekosistem, bukan hanya upstream source tree.
...
+-----------------------+----------------------+-------------------------------+
| Client-side injection | CVE-2025-61984       | Butuh untrusted username dan  |
|                       |                      | ProxyCommand                  |
+-----------------------+----------------------+-------------------------------+
| Downstream patch bug  | CVE-2026-3497        | Terkait distro GSSAPI delta   |
+-----------------------+----------------------+-------------------------------+
| Policy logic error    | CVE-2025-32728       | DisableForwarding tidak       |
|                       |                      | menutup semua forwarding      |
+-----------------------+----------------------+-------------------------------+
...
------------------------------------------------------------------------
  ...
  Upstream OpenSSH tidak berdiri sendirian ketika masuk ke Linux distribution.
  Paket distro dapat menambahkan patch, link tambahan, build flag, dan integrasi
  service manager. Di beberapa distro, systemd notification path membuat sshd
  terhubung tidak langsung ke libsystemd, lalu libsystemd dapat membawa liblzma.
  ...
  XZ Utils backdoor membuktikan bahwa dependency yang tidak tampak relevan bagi
...
------------------------------------------------------------------------
  ...
  [ Scope ]
    ...
    - Tidak memberi initial access remote.
    - Efektif sebagai persistence, bukan sebagai entry vector.

  Karena itu, teknik ini tidak sebanding dengan supply chain compromise modern.
  Ia tetap relevan sebagai pembanding historis: dulu targetnya jalur autentikasi
  langsung, sekarang targetnya dependency, build pipeline, dan edge behavior.
...
------------------------------------------------------------------------
  ...
  regreSSHion berbahaya karena berada pada server-side pre-authentication path.
  ...
  regreSSHion memberi tiga pelajaran:
...
------------------------------------------------------------------------
  ...
  Backdoor XZ menggunakan beberapa lapis penyamaran:
    ...
    1. Payload tersembunyi di test files.
  ...
  XZ mengubah baseline supply chain security:
...
------------------------------------------------------------------------
  ...
  ProxyCommand injection mengingatkan bahwa attack surface OpenSSH tidak hanya
  berada pada daemon. ssh client configuration, token expansion, URI parser,
  dan automation wrapper juga menjadi bagian dari security boundary.
...
------------------------------------------------------------------------
  ...
  Patch ini berguna secara operasional, tetapi juga memperluas attack surface.
...
------------------------------------------------------------------------
  ...
  Hal penting dari CVE ini bukan hanya bug GSSAPI. Yang penting adalah pola:
...
------------------------------------------------------------------------
  ...
  Dampak praktis:
    ...
    - policy hardening harus diuji, bukan hanya ditulis di config
...
------------------------------------------------------------------------
  ...
  Transisi post-quantum bukan hanya isu kriptografi teoretis. SSH sering dipakai
  untuk administrasi sistem bernilai tinggi. Data yang direkam hari ini dapat
  bernilai di masa depan jika masa kerahasiaannya panjang.
  ...
  Post-quantum transition tidak otomatis membuat OpenSSH lemah. Namun setiap
  transisi kriptografi besar selalu membawa parser baru, negotiation behavior
  baru, compatibility branch baru, dan downgrade surface baru.
...
+----------------------+-----------------------+-------------------------------+
| Initial access       | Root sudah ada        | Bisa remote, supply chain,    |
|                      |                       | atau client-side input path   |
+----------------------+-----------------------+-------------------------------+
| Detection            | Hash/package check    | Build artifact diff, runtime  |
|                      |                       | maps, anomaly telemetry       |
+----------------------+-----------------------+-------------------------------+
| Scope                | Single host           | Distro/package population     |
+----------------------+-----------------------+-------------------------------+
| Trigger              | Login credential      | Pre-auth signal, dynamic link,|
|                      |                       | token expansion, KEX behavior |
+----------------------+-----------------------+-------------------------------+
| Key risk             | Persistence           | Initial access, supply chain, |
|                      |                       | policy bypass, downgrade      |
+----------------------+-----------------------+-------------------------------+
...
+--------------------------------+------------+----------+----------------------+
| PQ downgrade/weak KEX policy   | No         | Long-term| High                 |
+--------------------------------+------------+----------+----------------------+
...
------------------------------------------------------------------------
  ...
  [ Forwarding Controls ]
    ...
    - Uji policy forwarding dengan test case, bukan hanya membaca config.
...
------------------------------------------------------------------------
  ...
  [ 4. Privilege Boundary Review ]
    ...
    - Petakan data yang bergerak dari unprivileged child ke monitor/root side.
...
------------------------------------------------------------------------
  ...
  Evolusi serangan terhadap OpenSSH menunjukkan perubahan fokus yang jelas.
  Patch manual terhadap auth path membutuhkan root dan mudah meninggalkan jejak.
  Serangan modern mencari ruang yang lebih sulit dipantau: signal handler,
  dependency chain, build artifact, downstream patch, client-side expansion,
  forwarding behavior, dan cryptographic negotiation.

  regreSSHion membuktikan bahwa regression testing untuk security bug lama
  tetap penting. Bug yang sudah ditutup dapat kembali ketika invariant lama
  hilang dari code path baru.

  XZ Utils backdoor membuktikan bahwa source review saja tidak cukup. Release
  artifact, generated build files, maintainer trust, dan dependency loading path
  harus masuk ke threat model.

  ProxyCommand injection membuktikan bahwa ssh client juga menjadi attack
  surface. Automation yang membangun ssh command dari input tidak tepercaya
  dapat membuat bug kecil berubah menjadi command execution path.

  CVE-2026-3497 dan CVE-2025-32728 menunjukkan bahwa distro patch dan policy
  directive harus diuji seperti code security boundary. Upstream OpenSSH yang
  kuat tidak otomatis membuat paket downstream bebas risiko.

  Transisi post-quantum KEX menambah pekerjaan baru untuk defender. Organisasi
  perlu memantau downgrade, compatibility exception, dan algoritma yang belum
  memiliki umur audit sepanjang RSA, ECDSA, atau X25519.

  Pelajaran akhirnya sederhana: OpenSSH tidak bisa lagi diaudit hanya sebagai
  satu daemon. Ia harus dilihat sebagai ekosistem runtime, build chain, distro
  patchset, client workflow, forwarding mechanism, dan cryptographic policy.
...
------------------------------------------------------------------------
  Kecoak Elektronik  |  Underground since 1995
  EOF
------------------------------------------------------------------------
